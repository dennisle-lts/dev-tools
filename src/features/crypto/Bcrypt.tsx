...
import { SALT_DEFAULT, SALT_MAX, SALT_MIN, compareStrings, hashString } from './bcrypt';
...
export default function Bcrypt() {
	...
	function adjustSalt(delta: number) {
		setSaltRounds((prev) => Math.min(SALT_MAX, Math.max(SALT_MIN, prev + delta)));
	}
				...
				<CardContent className='space-y-4 pt-6'>
					...
					{matchResult !== null && (
						<div className='flex items-center gap-2 rounded-md bg-muted px-3 py-3'>
							<span className='text-sm text-muted-foreground'>Do they match?</span>
							<span
